...
- **MySQL dialect compatibility for the wire-protocol path** (`services/mysqlCompat.ts`). Scoped deliberately: measured against ClickHouse 25.8, most of MySQL's function library already works unchanged (`DATE_ADD`, `DATE_SUB`, `DATE_FORMAT` with `%` specifiers, `STR_TO_DATE`, `TIMESTAMPDIFF`, `YEARWEEK`, `IFNULL`, `CONCAT`, `SUBSTRING_INDEX`, `LOCATE`, `GREATEST`, `ANY_VALUE`, `LIMIT n,m`, window functions, CTEs). What genuinely fails is now rewritten: `CURRENT_DATE`/`CURRENT_TIMESTAMP` (bare keywords parse as identifiers), `UNIX_TIMESTAMP`, `DATEDIFF(a,b)`, `CONVERT_TZ` from UTC, `WEEKDAY`, `DAYNAME`, `FIELD`, `GROUP_CONCAT ... SEPARATOR`, `JSON_UNQUOTE(JSON_EXTRACT(...))`, and `CAST(... AS SIGNED/UNSIGNED)`. Constructs with no faithful translation (`JSON_TABLE`, `JSON_OBJECT`, `JSON_CONTAINS`, `ON DUPLICATE KEY UPDATE`, `FOR UPDATE`, non-UTC `CONVERT_TZ`) are detected and logged with the reason instead of being silently mistranslated.
- **`CLICKHOUSE_JOIN_USE_NULLS`** (default on). MySQL fills unmatched `LEFT`/`RIGHT JOIN` columns with `NULL`; ClickHouse defaults to the column's zero value (`0`, `''`, `1970-01-01`), which silently breaks `IS NULL` checks, `COALESCE`, and aggregates that should skip missing rows. Found by differential testing and now corrected at the shared read path. Trade-off, recorded as a test: with the setting on, ClickHouse rejects a `LEFT JOIN` whose `ON` clause carries an inequality spanning both tables — such joins need rewriting as an `INNER JOIN` or a `WHERE` predicate. Failing loudly beats returning `0` where `NULL` was meant; set `CLICKHOUSE_JOIN_USE_NULLS=false` to revert.
- **Differential test suite** (`mysqlCompatDifferential.test.ts`, opt-in via `DUCKLING_DIFFERENTIAL=1`): provisions MySQL 8.4 and ClickHouse 25.8 with identical data and asserts that 29 queries return **identical results** on both. Executing without error is a weak guarantee for a compatibility layer; this catches the silent-difference class. It already found two real bugs — `DATEDIFF` operand order (MySQL computes `a - b`, ClickHouse `end - start`, so a naive mapping negates every result) and `CAST(x AS UNSIGNED)` rounding (MySQL rounds 10.5 to 11, ClickHouse truncates to 10, and ClickHouse's `round()` is no help because it rounds half to even). Now 59 cases over 6 seeded rows including NULLs, negatives, empty strings and a year boundary. Beyond the two bugs above it also caught `TIMESTAMPDIFF` counting unit *boundaries* in ClickHouse versus *full units elapsed* in MySQL (`23:59:59` → `00:00:00` is 1 minute for `dateDiff`, 0 for MySQL — now mapped to `age()`), and `CONVERT_TZ` with numeric offsets (`'+00:00','+05:30'`) which no ClickHouse zone expresses and is now an `INTERVAL` shift. Three differences are **pinned as tests rather than papered over**: MySQL's default collation makes `'a' = 'A'` true while ClickHouse compares bytes; `ORDER BY` after `UNION ALL` orders the combined result on MySQL but only the last `SELECT` on ClickHouse; and the `join_use_nulls` constraint above.
- **Unit suite for the rewriter** (`mysqlCompat.test.ts`, 40 tests, no Docker required): string-literal inviolability, idempotence, nesting, arity guards, identifiers that share a name with a rewritten function, unbalanced parentheses, and byte-identical passthrough for queries needing nothing. Coverage of `mysqlCompat.ts` went from 43% statements / 33% branches to **90% / 84%**.

...
**MySQL zero dates (solved via patched images; safe by default, not yet upstreamed):** Stock PeerDB v0.36 corrupts MySQL `0000-00-00` values to `1970-01-01` on the ClickHouse path. The v3 patch (`docs/peerdb-upstream-zero-date-poc-v3.patch`) fixes this at the row-read layer with a `PEERDB_MYSQL_ZERO_DATE_AS_NULL` mirror setting (zero/partial-zero dates → NULL, matching duckling's polling-mode behavior; pair with `PEERDB_NULLABLE=true`). Verified end-to-end via `PEERDB_ZERO_DATE_AS_NULL=true tests/peerdb/run-type-coverage.sh`. **Bringing the stack up with `scripts/peerdb-up.sh` is zero-date-safe by default** — it builds the patched flow images from source on first run (`scripts/build-peerdb-zero-date-poc.sh`, ~one slow build, cached after) and pins them. The escape hatches still apply: a bare `docker compose -f docker-compose.peerdb.yml up` uses stock corrupting images, and the patched images are local-only (not pushed to a registry) until the patch is upstreamed. The separate `1000-01-01` → Date32 range clamp remains unfixed. The capability probe surfaces zero-dates as a `knownBlockers` entry on `/api/databases/:id/replication-mode`.

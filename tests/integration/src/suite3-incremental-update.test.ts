...
describe('Suite 3: Incremental Update', () => {
  ...
  test('Widget A quantity updated (incremental)', async () => {
    expect(await duckdbScalarStrict('SELECT quantity FROM products_simple WHERE id = 1', 'quantity')).toBe('80');
  });
  ...
  test('Alice role updated (full sync)', async () => {
    expect(await duckdbScalarStrict('SELECT role FROM users_with_timestamps WHERE id = 1', 'role')).toBe('editor');
  });

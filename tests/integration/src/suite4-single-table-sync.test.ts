...
describe('Suite 4: Single Table Sync', () => {
  ...
  test('products count after single-table sync', async () => {
    expect(await duckdbScalarStrict('SELECT COUNT(*) AS cnt FROM products_simple', 'cnt')).toBe('6');
  });

  test('users count unchanged by single-table sync', async () => {
    const usersAfter = await duckdbScalarStrict('SELECT COUNT(*) AS cnt FROM users_with_timestamps', 'cnt');
    expect(usersAfter).toBe(usersBefore);
  });

  test('events count unchanged by single-table sync', async () => {
    const eventsAfter = await duckdbScalarStrict('SELECT COUNT(*) AS cnt FROM events_append_only', 'cnt');
    expect(eventsAfter).toBe(eventsBefore);
  });

  test('Gadget F exists after single-table sync', async () => {
    expect(await duckdbScalarStrict('SELECT name FROM products_simple WHERE id = 6', 'name')).toBe('Gadget F');
  });
});

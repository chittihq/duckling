...
describe('Suite 5: Idempotent Re-sync', () => {
  ...
  test('events no duplicates after re-sync', async () => {
    expect(await clickhouseScalarStrict('SELECT COUNT(*) AS cnt FROM events_append_only', 'cnt')).toBe(eventsBefore);
  });

  test('products no duplicates after re-sync', async () => {
    expect(await clickhouseScalarStrict('SELECT COUNT(*) AS cnt FROM products_simple', 'cnt')).toBe(productsBefore);
  });

...
function seedRestartReproRows(rowCount: number, startId: number): void {
...
describe('Suite 14: Incremental Crash Probe', () => {
  test('default persisted-volume incremental probe does not exit with a segfault', async () => {
    const currentCount = Number(await duckdbScalarStrict(`SELECT COUNT(*) AS cnt FROM ${TABLE_NAME}`, 'cnt'));
    const nextId = currentCount + 1;
    const expectedFinalCount = currentCount + PROBE_INCREMENTAL_ROWS;

    seedRestartReproRows(PROBE_INCREMENTAL_ROWS, nextId);
    ...
    const finalCount = Number(await duckdbScalarStrict(`SELECT COUNT(*) AS cnt FROM ${TABLE_NAME}`, 'cnt'));
    const state = getDucklingContainerState();
